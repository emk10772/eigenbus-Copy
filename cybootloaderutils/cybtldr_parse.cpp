    ...
    *appSize = 0;
    ...
    do
    {
        ...
        if (err == CYRET_SUCCESS)
        {
            ...
            else if (rowLength >= APPINFO_META_HEADER_SIZE && strncmp((const char *)buf, APPINFO_META_HEADER, APPINFO_META_HEADER_SIZE) == 0)
            {
                // find seperator index
                seperatorIndex = strcspn((const char *)buf, APPINFO_META_SEPERATOR_START);
                if (strncmp((const char *)buf + seperatorIndex, APPINFO_META_SEPERATOR, APPINFO_META_SEPERATOR_SIZE) == 0)
                {
                    *appStart = 0;
                    *appSize = 0;
